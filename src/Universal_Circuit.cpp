...
void Eug1Test();
void circuitTest();
void UniversalCircuitTest(string,uint32_t,uint32_t ,uint32_t,bool );
...
int main(int argc, char* argv[])
{
	...
	*/
	...
	if(argc<2)
	{
		cout<<"Input format: -ufile filename.circuit"<<endl;
		cout<<"Input format: -sfile filename.circuit"<<endl;
		//cout<<"Input format: -random"<<endl;
		return 0;
	}
	string gatefilename;//path+filename
	...
	string gatefilename_mod;
	uint32_t gate_num = 0,input_num=0,output_num=0;
	cout<<"Input File  Name is "<<gatefilename<<endl; 
	if(sflag == false)
	{
		cout << endl << "START STANDARDIZED UC" << endl;
   		gate_num = SHDL_to_SHDL(gatefilename,input_num,output_num, sflag);//adder_32bit.txt_SHDL.circuit
    	cout << "END STANDARDIZED UC" << endl;
		gatefilename_mod = gatefilename+"_Mod_SHDL.circuit";
		gate_num = SHDL_to_SHDL(gatefilename_mod,input_num,output_num, !sflag)-1;
		//gate_num = SHDL_to_SHDL(gatefilename_mod,input_num,output_num, !sflag);
	}else{
		gatefilename_mod=gatefilename;
		gate_num = SHDL_to_SHDL(gatefilename,input_num,output_num, sflag)-1;
	}
	cout<<"gate_num:"<<gate_num<<endl;
	cout<<"input_num:"<<input_num<<endl;
	//cout<<"gatefilename_mod:"<<gatefilename_mod<<endl;
	UniversalCircuitTest(sfilename,gate_num,input_num,output_num,sflag);//
...
void UniversalCircuitTest(string fileName ,uint32_t gate_num,uint32_t input_num,uint32_t output_num,bool flag)
{	

	//string fileName = "Test/Test4Gate.circuit";
	//gate_num=12138;
	string gatefilename;
	if(flag == false)
	{
		gatefilename = "Test/"+fileName+"_Mod_SHDL.circuit";
	}else{
		gatefilename = "Test/"+fileName;
	}
	
	cout<<"fileName:"<<fileName<<endl;
	UC* uc = new UC(1,gate_num); // get UC
	cout<<"gate_num:"<<gate_num<<endl;
	
	cout<<"input_num:"<<input_num<<endl;
	bool* input = new bool[input_num];
	default_random_engine random_engine; 
	//bool* input = new bool[128];
 ...
 for (int i = 0; i < input_num/2; i++) {
  input[i] = a % 2;
  input[input_num/2 + i] = b % 2;
  a /= 2;
  b /= 2;
  //cout << input[i] << " + " << input[64 + i] << endl;
 }
	/* 
	...
	//input[7] = false;
	*/
	uc->setCircuit(gatefilename, FileType::FILE_TYPE_CIRCUIT); //set the value of X- and Y-Gate
	...
	*/
	...
	uc->setInput(input, input_num,fileName,gate_num);
 ...
 {
  if (uc->SubEug0->poleList[i]->dagNodeType == DAG_NODE_TAIL) 
  {
   cout << " " << uc->SubEug0->poleList[i]->controlledGate->output0;
   SumUC += power * uc->SubEug0->poleList[i]->controlledGate->output0;
   power *= 2;
  }

 }
 cout << endl;
 cout << "SumC = " << SumC << "    SumUC = " << SumUC;
 if (SumC == SumUC) 
 {
  cout << " Accept " << endl;
 }
